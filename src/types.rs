...
///
/// Three of them rather than two, because a `where let` variable can be a third
/// thing a declaration's parameter never can: the presence a `when` names. The
/// first two are the two a rest can be — the rest of a struct *is* a type, since
/// `..r` in a struct puts whatever is written for `r` in the type's core, and
/// only a sum's rest is a reading of its own, for the reason [`Rest`] gives.
///
/// [`ParamKind`] answers with the first two alone: a declaration binds no
/// presence, so nothing there can ever be read the third way.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum Sense {
    Type,
    Cases,
