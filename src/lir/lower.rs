...
impl Lower<'_> {
    ...
    fn host_to_ruddy(
        ...
        self.host_to_ruddy_protocol(
            conversion,
            ty,
            raw,
            body,
            crate::externs::Completion::Immediate,
        )
    }
    fn host_to_ruddy_protocol(
        ...
        match conversion {
            crate::externs::Conversion::Protocol {
                inner, completion, ..
            } => self.host_to_ruddy_protocol(inner, ty, raw, body, *completion),
    ...
    fn ordinary_extern_level(
        ...
        if matches!(&*ty, Ty::Named { .. })
            && let Some(adapter) = self.extern_adapters.iter().rev().find(|adapter| {
                adapter.completion == completion && same_finite_syntax(&adapter.ty, &ty)
            })
        {
            return adapter.id;
        }
        ...
        if recursive {
            self.extern_adapters.push(OrdinaryExternAdapter {
                ty: ty.clone(),
                id,
                completion,
            });
    ...
    fn ruddy_to_host(
        ...
        match conversion {
            crate::externs::Conversion::Protocol {
                ...
                let value = self.ruddy_to_host(inner, ty, value, available, body);
                self.emit(
                    body,
                    Span::default(),
                    Rep::Fn,
            ...
            crate::externs::Conversion::MarkedFunction {
                ...
                self.emit(
                    ...
                    Op::Callback {
                        value,
                        mode: crate::externs::Callback::Sync,
                    },
                )
            }
            crate::externs::Conversion::OrdinaryFunction if self.rep(ty) == Rep::Fn => {
