...
pub struct GuardedArm {
    ...
    /// Coverage or case-refinement batch identifying this arm's premise.
    pub premise_reason: ReasonId,
    pub constraints: Vec<Constraint>,
    pub requirements: Vec<DeferredRequirement>,
    /// Equality between this body's type and the match result. Structural
    /// matches solve it under the arm's premise; tag matches share an ordinary
    /// result type because several cases may be admitted simultaneously.
...
pub enum Rule {
    ...
    Witness,
    /// Two mirrors, decomposed into the one type each mirrors.
    Mirror,
    TypeInfo,
...
pub enum ConstraintKind {
    ...
    /// A structural-presence or finite tag match. Each arm owns its body
    /// constraints and store requirements. Structural conditions exclude prior
    /// arms; tag conditions independently require each admitted case's body.
    Match {
        scrutinee: Arc<Ty>,
        result: Arc<Ty>,
        arms: Vec<GuardedArm>,
...
fn describe_type(ty: &Arc<Ty>) -> TypeDescription {
    ...
    match &**ty {
        ...
        Ty::Mirror(..) => TypeDescription::Mirror,
        Ty::TypeInfo(..) => TypeDescription::TypeInfo,
...
fn smallest_incompatible_counted_with_mask(
    ...
    }) = work.pop()
    {
        ...
        match (&*left, &*right) {
            ...
            | (Ty::TypeInfo(left), Ty::TypeInfo(right)) => work.push(MismatchWork {
                left: left.clone(),
                right: right.clone(),
                alias_path,
                alias_work,
...
impl Table {
            ...
            match part {
                Work::Ty(a, b) => {
                    ...
                    match (&*a, &*b) {
                        ...
                        | (Ty::Mirror(element), Ty::Mirror(other))
                        | (Ty::TypeInfo(element), Ty::TypeInfo(other)) => {
            ...
            match opened {
                Opened::Ty(ty) => {
                    let shape = match &*ty {
                        ...
                        Ty::Mirror(..) => ProvenanceTy::Mirror,
                        Ty::TypeInfo(..) => ProvenanceTy::TypeInfo,
        ...
        while let Some(part) = work.pop() {
            match part {
                ...
                Work::Ty(ty) => match ty {
                    ...
                    | Ty::Mirror(body)
                    | Ty::TypeInfo(body)
                    | Ty::Hidden { body, .. } => work.push(Work::Ty(body)),
                    Ty::Arrow(from, to, row) => {
    ...
    /// Every nested field or case path a match demand names and the presence
    /// deciding it. Case steps carry `#`. Paths retain their parents so
    ...
    fn presence_paths(
        ...
        let ty = self.resolve(ty);
        if let Ty::Struct(row) | Ty::Sum(row) = &*ty {
            let sum = matches!(&*ty, Ty::Sum(_));
                ...
                match item {
                    Work::Ty(mut ty, _) => {
                        ...
                        let (tag, children) = match &*ty {
                            ...
                            Ty::Mirror(inner) => {
                                (ProvenanceTy::Mirror, vec![Work::Ty(inner.clone(), None)])
                            }
                            Ty::TypeInfo(inner) => {
                                (ProvenanceTy::TypeInfo, vec![Work::Ty(inner.clone(), None)])
                            }
                            Ty::Struct(row) => {
            ...
            let (shape, parent, old, children): (_, _, _, Vec<Published>) = match item {
                Published::Ty(ty, parent, old) => {
                    let (tag, child_types): (ProvenanceTy, Vec<Published>) = match &*ty {
                        ...
                        Ty::Mirror(inner) => (
                            ProvenanceTy::Mirror,
                            vec![Published::Ty(inner.clone(), None, None)],
                        ),
                        Ty::TypeInfo(inner) => (
                            ProvenanceTy::TypeInfo,
                            vec![Published::Ty(inner.clone(), None, None)],
                        ),
                        Ty::Struct(row) => (
                            ProvenanceTy::Struct,
    ...
    fn zonk(&self, ty: &Arc<Ty>, subst: &Subst) -> Arc<Ty> {
        ...
        while let Some(part) = work.pop() {
            match part {
                ...
                Work::Mirror => {
                    let element = types.pop().expect("zonked mirror type");
        ...
        match item {
            Build::Ty(ty) => {
                ...
                match &*ty {
                    ...
                    Ty::Mirror(inner) => {
                        work.push(Build::Mirror(wrap));
                        work.push(Build::Ty(inner.clone()));
                    }
                    Ty::TypeInfo(inner) => {
                        work.push(Build::TypeInfo(wrap));
                        work.push(Build::Ty(inner.clone()));
                    }
                    Ty::Struct(row) => {
            ...
            Build::Mirror(wrap) => {
                let value = Arc::new(Ty::Mirror(types.pop().unwrap()));
                types.push(if wrap {
                    Arc::new(Ty::Package(value))
                } else {
                    value
            ...
            Build::TypeInfo(wrap) => {
                let value = Arc::new(Ty::TypeInfo(types.pop().unwrap()));
                types.push(if wrap {
                    Arc::new(Ty::Package(value))
                } else {
                    value
...
fn shift(ty: &Arc<Ty>, by: u32) -> Arc<Ty> {
    ...
    while let Some(part) = work.pop() {
        match part {
            ...
            Work::Mirror => {
                let element = types.pop().expect("shifted mirror type");
                types.push(Arc::new(Ty::Mirror(element)));
            }
            Work::TypeInfo => {
                let element = types.pop().expect("shifted mirror type");
...
fn extern_annotation_sources(
    ...
    while let Some((written, mut semantic)) = work.pop() {
        ...
        match (&written.anchored, &*semantic) {
            ...
            | (ir::TypeKind::Mirror(element), Ty::Mirror(lowered))
            | (ir::TypeKind::TypeInfo(element), Ty::TypeInfo(lowered)) => {
...
fn lower_scoped(
    ...
    let lowered = match &ty.anchored {
        ...
        TypeKind::Mirror(element) => {
            Ty::Mirror(lower_scoped(mint, table, tails, element, boundaries))
        }
        TypeKind::TypeInfo(element) => {
            Ty::TypeInfo(lower_scoped(mint, table, tails, element, boundaries))
        }
...
fn substitute_type_with(
    ...
    while let Some(part) = work.pop() {
        match part {
            ...
            Work::Mirror => {
                let element = types.pop().expect("mirror substitution postorder");
                types.push(Arc::new(Ty::Mirror(element)));
            }
            Work::TypeInfo => {
                let element = types.pop().expect("mirror substitution postorder");
