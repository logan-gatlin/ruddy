...
Git repositories are fetched and checked out with pure-Rust `gix` and Rustls—Ruddy never invokes a Git executable. Checkouts live under `$RUDDY_HOME/git/checkouts`; if `RUDDY_HOME` is unset the default is `$XDG_CACHE_HOME/ruddy`, or `$HOME/.cache/ruddy`. Resolution can populate this cache even if compilation later fails. Cached trees are restored to their locked commit under a cross-process lock before every compilation. `compile` and `compile_graph` write no artifacts, but may fetch dependencies and update `Ruddy.lock`; `build` writes local artifacts only after the entire graph compiles and never writes into Git cache checkouts.
