...
fn a_later_overlapping_compile_from_the_same_browser_wins_its_own_race() {
    ...
    first.revision = 1;
    assert!(store.observe(first, 0, "browser".into(), SessionView::default()));

    let mut later = request("later");
    later.revision = 2;
    assert!(store.observe(later, 0, "browser".into(), SessionView::default()));
...
fn edits_must_name_the_open_document_file_and_current_revision() {
    ...
    store.observe(request(""), 0, "browser".into(), SessionView::default());
    assert_eq!(
        store
...
fn shared_view_tracks_what_the_human_is_inspecting() {
    ...
    store.observe(request(""), 0, "browser".into(), view.clone());
