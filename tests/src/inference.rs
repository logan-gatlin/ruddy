...
/// the errors, and the causal explanations a reporter renders, are the same
/// either way.
#[test]
fn a_complete_trace_is_the_only_thing_the_trace_setting_adds() {
    ...
    let explanation = |output: &inference::Output| {
        let [error] = output.errors() else {
            panic!("one mismatch: {:#?}", output.errors());
        };
        error
            .explanation
            .clone()
            .expect("a mismatch carries its causal explanation")
    };
...
fn arrays_infer_one_element_type_and_generalize_empty_literals() {
    ...
    assert_eq!(scheme(&mint, &output, "reals"), "[Real]");
    assert_eq!(scheme(&mint, &output, "strings"), "[String]");
}
...
fn positive_result_presences_can_forget_input_correlations() {
    ...
    let symbol = symbol_named(
        &mint,
        output.semantics().schemes().keys().copied(),
        "forget",
    );
    let Formula::And(input, output_formula) = output.semantics().schemes()[&symbol].formula()
    else {
...
fn negative_and_erased_alias_arguments_do_not_publish_anonymous_existentials() {
    ...
    for name in ["negative", "erased"] {
        let symbol = symbol_named(&mint, output.semantics().schemes().keys().copied(), name);
        let scheme = &output.semantics().schemes()[&symbol];
...
fn every_primitive_is_a_distinct_type() {
    ...
    let (mint, _, output) =
        inferred("let real = 1\nlet int = 1i\nlet nat = 1n\nlet decimal = 1.25");
    assert_eq!(scheme(&mint, &output, "real"), "Real");
    assert_eq!(scheme(&mint, &output, "int"), "Int");
    assert_eq!(scheme(&mint, &output, "nat"), "Nat");
    assert_eq!(scheme(&mint, &output, "decimal"), "Real");

    let (_, _, output) = infer_src("let bad : Int = 1");
    assert_eq!(
        output.errors()[0].kind.to_string(),
        "type mismatch: expected `Int`, found `Real`"
    );
}
...
fn scheme(mint: &Mint, inferred: &inference::Output, name: &str) -> String {
    let symbol = symbol_named(mint, inferred.semantics().schemes().keys().copied(), name);
...
fn constraints(mint: &Mint, inferred: &inference::Output, name: &str) -> Vec<String> {
    let symbol = symbol_named(
        mint,
        inferred.diagnostics().constraints().keys().copied(),
        name,
    );
    inferred.diagnostics().constraints()[&symbol]
        .iter()
...
fn mentions_a_variable(ty: &Ty) -> bool {
    match ty {
        Ty::Var(_) => true,
        Ty::Arrow(a, b, r) => {
            mentions_a_variable(a) || mentions_a_variable(b) || row_mentions_a_variable(r)
        }
        Ty::Struct(r) | Ty::Sum(r) => row_mentions_a_variable(r),
        Ty::Named { args, .. } => args.iter().any(|a| mentions_a_variable(a)),
        _ => false,
    }
}
...
fn a_literal_is_a_nat() {
    let (mint, out, output) = inferred("let n = 1n");
    assert_eq!(scheme(&mint, &output, "n"), "Nat");
...
fn a_lambda_argument_stays_monomorphic() {
    ...
    let (_, _, output) = infer_src("let both = fn f => { a: f 1n, b: f {} }");
    assert!(
        output
            .errors()
            .iter()
...
fn an_argument_mismatch_names_the_parameter_as_the_expectation() {
    ...
    let (_, _, output) = infer_src("let p : { x: Nat } = { x: 1n, y: 2n }");
    let [error] = output.errors() else {
        panic!("expected exactly one error: {:#?}", output.errors());
    };
    assert_eq!(
    ...
    let (_, _, output) = infer_src("let bad = 1n 1n");
    let [error] = output.errors() else {
        panic!("expected exactly one error: {:#?}", output.errors());
    };
    assert_eq!(
...
fn a_failed_definition_is_undecided_rather_than_polymorphic() {
    let (mint, _, output) = infer_src("let bad = 1n 1n\nlet ok : Nat = bad\nlet ok2 = bad 5n");
    assert_eq!(output.errors().len(), 1, "{:#?}", output.errors());
    ...
    let (mint, _, output) =
        infer_src("let f = fn x => (fn r => r.a) { b: x }\nlet g = f { b: 1n }");
    assert_eq!(output.errors().len(), 1, "{:#?}", output.errors());
...
fn a_tail_name_is_scoped_to_its_annotation() {
    ...
    let (mint, _, output) = inferred(
        "let f : { x: Nat, ..'r } -> Nat = fn p => p.x\n\
         ...
         let n = f { x: 1n, extra: 2n }\n\
         let m = g { y: 1n, other: 2n }",
    );
    assert_eq!(scheme(&mint, &output, "f"), "{ x: Nat, ..'a } -> Nat");
...
/// An annotation is the contract, and a `where 'let` variable is what makes it
/// one: it stands for whatever the caller picks, so nothing the body does may
/// decide it, and a body that tries is refused at the expression that tried.
///
/// This used to be a blanket complaint about the whole annotation, raised once
/// the group was solved and pointing at a type several lines up. What it is now
/// is the line the reader can change, said in what that line did.
#[test]
fn a_declared_variable_is_the_definitions_to_leave_alone() {
    ...
    let src = "let f : { ..'r } -> { x: Nat, ..'r } = fn p => { x: 0n }";
    let (_, _, output) = infer_src(src);
    let [error] = output.errors() else {
        panic!("expected exactly one error: {:#?}", output.errors());
    };
    ...
    // A value binding is a definition too, and a concrete value decides
    // whatever it is written under — so a declared variable over one is refused
    // wherever the value says what it is.
    let (_, _, output) = infer_src("let x : 'a = { a: 1n }");
    let [error] = output.errors() else {
        panic!("expected exactly one error: {:#?}", output.errors());
    };
    assert_eq!(error.kind.code(), "rigid-broken");

    // Two variables the definition made into one. Neither is decided by
    // anything the reader could point at — what went wrong is that the body
    // treats two independent promises as one — and each is refused against the
    // other where the body brought them together.
    for src in [
        "let f : { ..'r } -> { ..'s } -> { ..'r } = fn p => fn q => q",
        "let f : 'a -> 'b -> 'a = fn p => fn q => q",
    ] {
        let (_, _, output) = infer_src(src);
        let [error] = output.errors() else {
            panic!(
                "expected exactly one error for {src}: {:#?}",
                output.errors()
            );
        };
        assert_eq!(error.kind.code(), "rigid-broken", "{src}");
    }
}
...
fn an_annotation_the_definition_keeps_open_is_no_complaint() {
    ...
    let (mint, _, output) =
        inferred("let f : { ..'r } -> { ..'s } -> { ..'r } = fn p => fn q => p");
    assert_eq!(
        scheme(&mint, &output, "f"),
        "{ ..'a } -> { ..'b } -> { ..'a }"
    );
...
fn two_rows_sharing_a_tail_cannot_differ() {
    let (mint, _, output) = infer_src(
        "let g : { x: Nat, ..'r } -> { y: Nat, ..'r } -> Nat = fn a => fn b => 1n\n\
         let h = fn c => g c c",
    );
    assert_eq!(
...
fn a_tail_may_not_take_a_field_its_own_row_names() {
    // The tail `r` is named beside a `y` in one argument and stands alone in
    // another, so handing it a struct with a `y` in it asks `r` for a `y`.
    // This used to typecheck: the solve bound `r` to `{ y: {} }`, the two
    // copies of `y` never met again, and zonking dropped one of them without a
    // word — so the type nothing had checked was the one that was published.
    //
    // Reached through a call rather than through `h`'s own body, and that is
    // where the condition has to be said again: the scheme quantifies the
    // shared tail, and losing the condition on the way out of a scheme is how
    // this one used to be reported — as an incidental `Nat ~ {}` mismatch,
    // and only because the two rows happened to meet a second time. Inside the
    // body `r` is rigid instead, which is a stronger refusal in other words.
    let (_, _, output) = infer_src(
    ...
    let (_, _, output) = infer_src(
        "let h : { ..'r } -> { x: { a: Nat, b: Nat, ..'r } } -> Nat = fn a => fn b => 1n\n\
         let z = h { a: 1n, b: 2n } { x: { a: 1n, b: 2n } }",
    );
    let [error] = output.errors() else {
        panic!("expected exactly one error: {:#?}", output.errors());
    };
...
fn a_row_repeating_two_fields_is_one_complaint() {
    ...
    let (_, out, output) = infer_src(
        "let h : { x: Nat, y: Nat, ..'r } -> { ..'r } -> Nat = fn a => fn b => 1n\n\
         let k = fn p q => h p { x: 1n, y: 2n }",
    );
    assert!(out.errors.is_empty(), "ir errors: {:#?}", out.errors);
    assert_eq!(output.errors().len(), 1, "{:#?}", output.errors());
    assert!(matches!(
...
fn what_a_tail_may_not_be_is_carried_across_a_binding() {
    ...
    let (_, _, output) = infer_src(
        "let h : { y: Nat, ..'r } -> { ..'r } -> Nat = fn a => fn b => 1n\n\
...
fn a_projection_needs_nothing_said_about_its_base() {
    ...
    let (mint, _, output) = inferred("let h : Nat = (fn p => p.x) { x: 1n }");
...
fn nested_projections_solve_in_one_pass() {
    ...
    let (mint, _, output) =
        inferred("let deep : Nat = (fn q => (fn p => p.x) q.inner) { inner: { x: 1n } }");
...
fn a_type_may_name_itself() {
    ...
    let list = symbol_named(&mint, output.semantics().aliases().keys().copied(), "list");
    assert_eq!(
        output.semantics().aliases()[&list].to_string(),
        "{ val: Nat, next: list }"
    );
}
...
fn recursive_types_may_be_written_in_terms_of_each_other() {
    ...
    let tree = symbol_named(&mint, output.semantics().aliases().keys().copied(), "tree");
    assert_eq!(
        output.semantics().aliases()[&tree].to_string(),
        "{ val: Nat, kids: forest }"
    );
}
...
fn a_declared_type_stays_the_name_it_was_written_as() {
    ...
    let pair = symbol_named(&mint, output.semantics().aliases().keys().copied(), "Pair");
    assert_eq!(
        output.semantics().aliases()[&pair].to_string(),
        "{ f: Endo }"
    );
...
fn alias_row_opening_is_total_for_every_semantic_argument_exit() {
    ...
    for (arg, expected) in [
        ...
        let opened = inference::unfold(output.semantics().aliases(), &named(Some(arg)));
    ...
    let opened = inference::unfold(output.semantics().aliases(), &named(None));
    assert!(matches!(
...
fn transitive_forwarding_bodies_do_not_spend_growth_fuel() {
    ...
    assert!(output.diagnostics().steps().iter().any(|step| {
        matches!(
            &step.goal,
...
fn alias_unfolding_is_linear_and_stack_safe_for_deep_forwarding() {
    std::thread::Builder::new()
        ...
        .spawn(|| {
            ...
            assert!(matches!(
                &*inference::unfold(output.semantics().aliases(), &deep),
                Ty::Nat
            ));
        })
...
fn the_solve_is_recorded_rule_by_rule() {
    ...
    let (mint, _, output) = inferred("let w : Nat = 1n");
...
fn closing_a_row_from_both_sides_agrees_with_itself() {
    ...
    let (mint, _, output) = infer_src("let p : { a: Nat } = { b: 1n }");
    assert_eq!(
        steps(&mint, &output, "p"),
        [
    ...
    let (mint, _, output) = infer_src(
        "let f : (#A Nat) -> Nat = fn p => 1n\n\
         let g : (#B Nat) -> Nat = fn p => 1n\n\
...
fn a_struct_matches_on_its_field_names_in_both_passes() {
    ...
    let (_, _, output) = infer_src("let p : { a: Nat } = { a: 1n, b: 2n, c: 3n }");
...
fn a_constructor_stands_for_its_body_with_its_arguments_put_in() {
    let (mint, _, output) = inferred(
        "type Pair 'A 'B = { first: 'A, second: 'B }\n\
         let p : Pair Nat Nat = { first: 1n, second: 2n }",
    );
    ...
    let (_, _, output) = infer_src(
        "type Pair 'A 'B = { first: 'A, second: 'B }\n\
         let p : Pair Nat Nat = { first: 1n, second: {} }",
    );
...
fn a_row_inside_an_argument_still_lacks_what_it_names() {
    let (_, _, output) = infer_src(
        "type Box 'A = { it: 'A }\n\
         let f : Box { x: Nat, ..'r } -> { ..'r } -> Nat = fn b => fn c => 1n\n\
         let z = fn b => f b { x: 1n }",
    );
    assert_eq!(output.errors().len(), 1, "errors: {:#?}", output.errors());
...
fn no_solver_variable_survives_inside_an_argument() {
    ...
    for scheme in output.semantics().schemes().values() {
        assert!(
...
fn a_row_argument_may_not_repeat_a_field_the_type_names() {
    ...
    let (_, _, output) = infer_src(
        "type WithX 'r = { x: Nat, ..'r }\n\
         let g : WithX { ..'s } -> { ..'s } -> Nat = fn p => fn q => 1n\n\
...
fn malformed_non_nominal_recursive_growth_is_absorbed() {
    ...
    assert!(
        output
            ...
            .steps()
            .iter()
            .any(|step| step.rule == Rule::Absorb),
        "steps: {:#?}",
        output.diagnostics().steps()
    );
}
...
fn a_tag_is_open_where_a_struct_literal_is_closed() {
    let (mint, _, out) = inferred("let v = #Some 1n");
...
fn a_declaration_may_take_the_rest_of_a_sum() {
    ...
    let (_, out, _) = infer_src(
        "type Fallible 'r = #Err Nat | ..'r\n\
         let bad : Fallible (#Err Nat) = #Err 1n",
    );
    assert_eq!(out.errors.len(), 1, "{:#?}", out.errors);
    assert!(matches!(
        &out.errors[0].kind,
...
fn a_case_may_be_marked_optional() {
    ...
    // A presence is not skolemized — the clause and the store are what govern
    // one — so a definition that settles it is no complaint at all, and the
    // scheme says what the definition turned out to be.
    let (mint, _, out) = inferred("let m : #A (when 'a) Nat | #B = #A 1n");
...
fn a_sum_may_name_itself() {
    let (mint, _, out) = inferred(
        "type List 'a = #Nil | #Cons { head: 'a, tail: List 'a }\n\
         ...
         let one : List Nat = #Cons { head: 1n, tail: empty }",
    );
    assert_eq!(scheme(&mint, &out, "empty"), "List Nat");
    assert_eq!(scheme(&mint, &out, "one"), "List Nat");
    let symbol = symbol_named(&mint, out.semantics().aliases().keys().copied(), "List");
    assert_eq!(
        out.semantics().aliases()[&symbol].to_string(),
...
fn a_sum_tail_may_not_come_back_naming_its_own_case() {
    let (_, out, output) = infer_src(
        "let h : (#A Nat | ..'r) -> (| ..'r) -> Nat = fn a => fn b => 1n\n\
         let k = fn p => h p (#A 1n)",
    );
    assert!(out.errors.is_empty(), "ir errors: {:#?}", out.errors);
    assert_eq!(output.errors().len(), 1, "{:#?}", output.errors());
...
fn a_field_settled_absent_is_no_repeat_for_a_tail_that_lacks_it() {
    inferred(
        "let opt : { x when 'a: Nat, .. } -> Nat = fn p => 1n\n\
         let closed : {} -> Nat = fn p => 1n\n\
         let g : { x: Nat, ..'r } -> { ..'r } -> Nat = fn a => fn b => 1n\n\
    ...
    inferred(
        "let opt : { x when 'a: Nat, .. } -> Nat = fn p => 1n\n\
         let closed : {} -> Nat = fn p => 1n\n\
         let g : { ..'r } -> { x: Nat, ..'r } -> Nat = fn a => fn b => 1n\n\
...
fn a_case_settled_absent_is_no_repeat_for_a_tail_that_lacks_it() {
    inferred(
        "let opt : (#A (when 'a) Nat | ..) -> Nat = fn p => 1n\n\
         let only : (#B Nat) -> Nat = fn p => 1n\n\
         let g : (#A Nat | ..'r) -> (| ..'r) -> Nat = fn a => fn b => 1n\n\
...
fn an_abandoned_presence_and_an_abandoned_tail_absorb_what_meets_them() {
    ...
    assert!(
        taken.contains(&"absorb  ? ~ { x when ?0: Nat, ..?1 } => no change".to_string()),
    ...
    let (mint, out, output) = infer_src(
        "let g : Nat -> (#A Nat) -> Nat = fn a => fn b => 1n\n\
         let f : (#A Nat | ..) -> Nat = fn p => g (nope p) p",
    );
    assert_eq!(out.errors.len(), 1, "{:#?}", out.errors);
    assert!(output.errors().is_empty(), "{:#?}", output.errors());
    let taken = steps(&mint, &output, "f");
    ...
    // actual.
    let (mint, out, output) = infer_src(
        "let g : Nat -> { } -> Nat = fn a => fn b => 1n\n\
         let f : { x when _: Nat, .. } -> Nat = fn p => g (nope p) p",
    );
    assert_eq!(out.errors.len(), 1, "{:#?}", out.errors);
    assert!(output.errors().is_empty(), "{:#?}", output.errors());
    let taken = steps(&mint, &output, "f");
...
fn two_presences_that_already_agree_need_no_rule_of_their_own() {
    ...
    let (mint, _, output) = inferred(
        "let optional : { x when 'a: Nat, y: Nat, .. } -> Nat = fn p => 1n\n\
         let closed : { y: Nat } -> Nat = fn p => 1n\n\
         let both : { y: Nat, ..'s } -> { y: Nat, ..'s } -> Nat = fn p => fn q => 1n\n\
...
fn a_rest_that_stands_for_everything_forbids_nothing() {
    let (mint, _, output) =
        inferred("type Only 'r = { ..'r }\nlet v : Only { x: Nat } = { x: 1n }");
...
fn an_assumption_matches_through_a_variable_the_two_sides_share() {
    let (mint, _, output) = inferred(
        "type Tree 'a = { value: 'a, kids: Nest 'a }\n\
         ...
         let peek : Tree { x when 'a: Nat } -> Nat = fn t => 1n\n\
         let take : Wood { x when 'b: Nat } -> Nat = fn w => 2n\n\
...
fn an_assumption_compares_arguments_of_every_shape() {
    ...
    let (mint, _, output) = inferred(
        "type A 'r = { next: B 'r, ..'r }\n\
         type B 'r = { next: A 'r, ..'r }\n\
         let use : { ..'s } -> A { ..'s } -> B { ..'s } -> Nat = fn c => fn x => fn y => 1n\n\
    ...
    let (mint, _, output) = inferred(
        "type A 'a = { v: 'a, next: B 'a }\n\
         ...
         let f : A (#X | #Y) -> Nat = fn p => 1n\n\
         let g : B (#X | #Y) -> Nat = fn p => 1n\n\
    ...
    let (mint, _, output) = inferred(
        "type A 'r = { next: B 'r, ..'r }\n\
         type B 'r = { next: A 'r, ..'r }\n\
         let use : A { p: Nat, ..'s } -> B { p: Nat, ..'s } -> Nat = fn x => fn y => 1n\n\
         let n = fn z => use z z",
    );
...
fn two_rows_sharing_a_tail_and_naming_the_same_fields_agree() {
    let (mint, _, output) = inferred(
        "let f : { x: Nat, ..'r } -> { x: Nat, ..'r } -> Nat = fn a => fn b => 1n\n\
         let g = fn p => f p p",
    );
    ...
    for src in [
        "let f : { x: Nat, ..'r } -> { x: Nat, y: Nat, ..'r } -> Nat = fn a => fn b => 1n\n\
         let g = fn p => f p p",
        // Whichever side is the longer one: the extras are the same cycle from
        // either end.
        "let f : { x: Nat, y: Nat, ..'r } -> { x: Nat, ..'r } -> Nat = fn a => fn b => 1n\n\
         let g = fn p => f p p",
    ] {
        let (_, _, output) = infer_src(src);
        let [error] = output.errors() else {
...
fn an_assumption_is_not_reused_at_an_argument_it_was_not_pushed_for() {
    ...
    let (mint, _, output) = inferred(
        "type Tree 'x = { v: 'x, k: Bush }\n\
         ...
         let g : Tree { a when 'a: Nat } -> Nat = fn x => 1n\n\
         let h : Tree2 { a when 'b: Nat } -> Nat = fn x => 2n\n\
...
fn a_recursive_use_is_monomorphic() {
    let (mint, _, output) = inferred("let self = fn x => self 1n");
...
fn a_definition_outside_a_group_stays_polymorphic() {
    let (mint, _, output) = inferred("let id = fn x => x  let a = id 1n  let b = id { x: 1n }");
    assert_eq!(scheme(&mint, &output, "id"), "'a -> 'a");
    assert_eq!(scheme(&mint, &output, "a"), "Nat");
...
fn a_forward_reference_instantiates_a_scheme() {
    let (mint, _, output) = inferred("let a = id 1n  let b = id { x: 1n }  let id = fn x => x");
    assert_eq!(scheme(&mint, &output, "id"), "'a -> 'a");
    assert_eq!(scheme(&mint, &output, "a"), "Nat");
    assert_eq!(scheme(&mint, &output, "b"), "{ x: Nat }");
}
...
fn the_schemes_are_keyed_in_source_order() {
    let (mint, out, output) = inferred("let a = b 1n  let b = fn n => c n  let c = fn n => n");
    ...
    assert_eq!(
        keys(output.semantics().schemes().keys().copied().collect()),
        source
    );
    assert_eq!(
        keys(output.diagnostics().constraints().keys().copied().collect()),
        source
    );
    ...
    for step in output.diagnostics().steps() {
...
/// A recursive definition deciding what its own annotation left as a hole is
/// no complaint at all: a hole is there to be decided, and a `when _` is one.
/// What the definition promised — its declared variables — is what it is held
/// to, and a body that keeps them is accepted however it recurses.
#[test]
fn a_recursive_definition_may_decide_the_holes_it_left() {
    let (mint, _, output) = infer_src("let f : { x when _: Nat } -> Nat = fn r => f { x: r.x }");
    assert!(output.errors().is_empty(), "{:#?}", output.errors());
    assert_eq!(scheme(&mint, &output, "f"), "{ x: Nat } -> Nat");

    // A failure inside it is still the one thing reported.
    let (_, _, output) = infer_src("let f : { x when _: Nat } -> Nat = fn r => f { x: r.y }");
    assert_eq!(output.errors().len(), 1, "{:#?}", output.errors());
    assert_eq!(output.errors()[0].kind.code(), "missing-field");
}
...
fn a_row_and_a_presence_are_abandoned_in_their_own_sorts() {
    ...
    let (mint, _, output) = infer_src("let bad : { y when 'a: Nat } = { y: 1n }.x");
    let recovered: Vec<String> = output
        ...
        .collect();
    assert!(!recovered.is_empty(), "{:#?}", output.diagnostics().steps());
    let _ = mint;
...
fn a_written_struct_lowers_to_unit_and_a_written_sum_to_a_sum_core() {
    ...
    let rec = symbol_named(&mint, output.semantics().aliases().keys().copied(), "Rec");
    let body = output.semantics().aliases()[&rec].body();
    assert!(
        matches!(&**body, Ty::Struct(row) if row.labels.len() == 1),
        "{body:?}"
    );

    let sum = symbol_named(&mint, output.semantics().aliases().keys().copied(), "Sum");
    let body = output.semantics().aliases()[&sum].body();
    let Ty::Sum(cases) = &**body else {
...
fn the_labels_of_two_types_are_decided_by_what_their_cores_allow() {
    ...
    let (mint, _, output) = inferred(
        "let one : { x: Nat, ..'r } -> Nat = fn p => 1n\n\
         let two : { x: Nat, y: Nat, ..'s } -> Nat = fn p => 1n\n\
    ...
    let (mint, _, output) = inferred(
        "let one : { x: Nat, ..'r } -> Nat = fn p => 1n\n\
         let two : { x: Nat, y: Nat } -> Nat = fn p => 1n\n\
         let both = fn v => { a: one v, b: two v }",
    );
    assert_eq!(
        scheme(&mint, &output, "both"),
    ...
    let (_, _, output) = infer_src("let a : { x: Nat } = { x: 1n, y: 2n }");
    assert_eq!(output.errors().len(), 1, "{:#?}", output.errors());
...
fn a_sums_tail_is_decided_as_a_row() {
    ...
    let (mint, _, output) = inferred(
        "let f : (#A Nat | ..'r) -> Nat = fn p => 1n\n\
         let g : (#A Nat | ..'s) -> Nat = fn p => 1n\n\
         let h = fn v => { one: f v, two: g v }",
    );
    assert_eq!(
        scheme(&mint, &output, "h"),
    ...
    // A recursive call instantiates what the annotation declared, so the tail
    // its own argument stands at takes the fresh copy — polymorphic recursion
    // over a declared rest, said in one step.
    ...
    // Two tails that really are the same variable are already the same thing.
    let (mint, _, output) = inferred("let f = fn p => match p with | #A x => f p | y => 1n end");
    let taken = steps(&mint, &output, "f");
    assert!(
        taken.iter().any(|step| step.contains("same  ?")),
        "{taken:#?}"
    );
    ...
    let (mint, out, output) = infer_src(
        "let mk : Nat -> (#A Nat) = fn n => #A n\n\
         let f : (#A Nat | ..) -> Nat = fn p => f (mk (nope p))",
    );
    assert_eq!(out.errors.len(), 1, "{:#?}", out.errors);
    let taken = steps(&mint, &output, "f");
    assert!(
    ...
    let (mint, out, output) = infer_src(
        "let g : Nat -> (#A Nat | #B) -> Nat = fn a => fn b => 1n\n\
         let f : (#A Nat | ..) -> Nat = fn p => g (nope p) p",
    );
    assert_eq!(out.errors.len(), 1, "{:#?}", out.errors);
    assert!(output.errors().is_empty(), "{:#?}", output.errors());
...
/// A sum's bare `..` in an annotation is a hole: a variable this definition may
/// decide, and deciding it is what a hole is for. A *declared* rest is the
/// opposite promise, and a body that decides one is refused at the expression
/// that decided it.
#[test]
fn a_sums_open_tail_is_the_definitions_to_leave_alone() {
    let (mint, _, output) = inferred("let f : (#A Nat | ..) -> Nat = fn p => 1n");
    assert_eq!(scheme(&mint, &output, "f"), "#A Nat | ..'a -> Nat");

    // A hole the body names another case in is decided, and the scheme says so.
    let (mint, _, output) = inferred("let f : (#A Nat | ..) -> Nat = fn p => f (#B 1n)");
    assert_eq!(scheme(&mint, &output, "f"), "#A Nat | #B Nat | ..'a -> Nat");

    // Declared, the tail is a promise instead — and a recursive use is a fresh
    // copy of it, so naming another case in one is polymorphic recursion rather
    // than a mistake.
    ...
    // nothing more is a promise broken too.
    let (_, _, output) = infer_src(
        "let g : (#A Nat) -> Nat = fn p => 1n\n\
         let f : (#A Nat | ..'r) -> Nat = fn p => g p",
    );
    assert_eq!(output.errors().len(), 1, "{:#?}", output.errors());
...
fn an_assumption_compares_a_sum_argument_as_a_sum() {
    let (mint, _, output) = inferred(
        "type Tree 'a = { value: 'a, kids: Nest 'a }\n\
         ...
         let f : Tree (#A Nat | ..'r) -> Nat = fn t => 1n\n\
         let g : Wood (#A Nat | ..'r) -> Nat = fn w => 1n\n\
         let h = fn v => { one: f v, two: g v }",
    );
    assert!(output.errors().is_empty(), "{:#?}", output.errors());
    let taken = steps(&mint, &output, "h");
...
/// A nested annotation is held to the same promise a definition's is, and to
/// the same reading of what a promise *is*: its declared variables are rigid,
/// and what it left open with a `..`, a `when _` or a `_` is a hole the value
/// is welcome to decide.
#[test]
...
fn a_nested_let_is_said_as_two_constraint_kinds() {
    ...
    let ConstraintKind::Let {
        level, value, body, ..
    } = &output.diagnostics().constraints()[&symbol][0].kind
    else {
...
fn positions(ty: &Ty) -> Vec<(u32, Sense)> {
    ...
    match ty {
        Ty::Bound(at) => found.push((*at, Sense::Type)),
        Ty::Arrow(a, b, r) => {
...
fn positions_row(row: &ruddy::types::Row, sense: Sense, found: &mut Vec<(u32, Sense)>) {
    for f in row.labels.values() {
        if let Presence::Bound(at) = f.presence {
            found.push((at, Sense::Presence))
        }
        found.extend(positions(&f.ty))
    }
    if let Rest::Bound(at) = row.rest {
        found.push((at, sense))
    }
}
...
fn a_written_absence_lowers_to_an_absent_presence() {
    ...
    let symbol = symbol_named(&mint, output.semantics().schemes().keys().copied(), "f");
    let body = output.semantics().schemes()[&symbol].body().clone();
    let Ty::Arrow(from, _, _) = &*body else {
        panic!("expected an arrow, got {body:?}");
    };
    let Ty::Struct(fields) = &**from else {
        panic!("expected struct")
    };
    let field = &fields.labels["y"];
    assert!(matches!(field.presence, Presence::Absent), "{field:?}");
    assert!(matches!(&*field.ty, Ty::Undecided), "{field:?}");

    // The sum counterpart, through the row inside the core.
    let (mint, _, output) = inferred("let s : #Ok Nat | \\#Err | .. = #Ok 1n");
    let symbol = symbol_named(&mint, output.semantics().schemes().keys().copied(), "s");
    let body = output.semantics().schemes()[&symbol].body().clone();
    let Ty::Sum(row) = &*body else {
        panic!("expected a sum, got {body:?}");
    };
    let case = &row.labels["Err"];
    assert!(matches!(case.presence, Presence::Absent), "{case:?}");
    assert!(matches!(&*case.ty, Ty::Undecided), "{case:?}");
}
...
fn a_shared_rest_lacks_the_absent_label() {
    // Two sides sharing one tail may not differ in labels — whatever the tail
    // absorbed from one it would grow on the other — except in labels
    // certainly *absent*, which say nothing a value could have. Reached
    // through a call, where the shared tail is a variable again: inside the
    // body that declared it, a rest is rigid and shares nothing.
    let (mint, _, output) = inferred(
        "let g : { \\y, ..'r } -> { ..'r } -> Nat = fn a => fn b => 1n\n\
         let h = fn c => g c c",
    );
...
fn a_partially_handled_case_stays_present_for_the_catch_all() {
    let (mint, _, output) = inferred(
        "let keep = fn r => 0n\n\
         let f = fn e => match e with | #A #X x => 1n | r => keep r end",
    );
...
fn a_natural_match_is_about_numbers() {
    let (mint, _, output) =
        inferred("let describe = fn n => match n with | 0n => #Zero | 1n => #One | k => #Many end");
    assert_eq!(
        scheme(&mint, &output, "describe"),
        "Nat -> #Zero | #One | #Many | ..'a"
    );
    let (mint, _, output) = inferred("let same = fn n => match n with | 0n => 1n | k => k end");
...
fn an_error_in_an_arm_body_is_reported_once() {
    let (_, _, output) = infer_src(
        "let wants_nat : Nat -> Nat = fn n => n\n\
         let f = fn e => match e with | #A #X x => 1n | r => wants_nat {} end",
    );
    assert_eq!(output.errors().len(), 1, "{:#?}", output.errors());
    assert!(matches!(
        output.errors()[0].kind,
...
fn a_natural_match_without_a_default_still_types() {
    let (mint, out, output) = infer_src("let f = fn n => match n with | 0n => 1n end");
    assert!(out.errors.is_empty(), "{:#?}", out.errors);
    assert!(output.errors().is_empty(), "{:#?}", output.errors());
...
fn a_catch_all_keeps_nested_rows_open() {
    let (mint, _, output) = inferred(
        "let f = fn e => match e with | #A #B #C 0n => 1n | #A #B r => 2n | q => 3n end\n\
         let user = f (#A #Z)",
    );
    assert_eq!(
        scheme(&mint, &output, "f"),
...
fn a_catch_all_keeps_struct_payload_rows_open() {
    let (mint, _, output) = inferred(
        "let f = fn e => match e with | #P #A { x: #X } => 1n | #P #A w => 2n | q => 3n end\n\
         let user = f (#P #Z)",
    );
    assert_eq!(
        scheme(&mint, &output, "f"),
...
fn an_overlapping_struct_arm_keeps_its_field_open() {
    let (mint, _, output) = inferred(
        "let f = fn e => match e with | { a: #A, b: #X } => 1n | { a: #A, b: k } => 2n end\n\
         let u = f { a: #A, b: #Z }",
    );
    ...
    let (_, _, output) = infer_src(
        "let wants_nat : Nat -> Nat = fn n => n\n\
         let f = fn e => match e with | { a: #A, b: 0n } => 1n | { a: #A, b: k } => wants_nat {} end",
    );
    assert_eq!(output.errors().len(), 1, "{:#?}", output.errors());
    assert!(matches!(
        output.errors()[0].kind,
...
fn a_column_closes_over_every_listed_case() {
    ...
    let (mint, _, output) = inferred(
        "let f = fn e => match e with \
         | { a: #A, b: 0n } => 1n | { a: #B, b: x } => 2n | { a: #A, b: k } => 3n end",
    );
    assert_eq!(scheme(&mint, &output, "f"), "{ a: #A | #B, b: Nat } -> Nat");
    ...
    let (mint, _, output) = inferred(
        "let f = fn e => match e with \
         | #T { a: #A, b: x } => 1n | #T { a: #B, b: 0n } => 2n | #T { a: #B, b: k } => 3n end",
    );
    assert_eq!(
        scheme(&mint, &output, "f"),
        "#T { a: #A | #B, b: Nat } -> Nat"
    ...
    for src in [
        "let f = fn e => match e with \
         | { a: #A, b: x } => 1n | { a: #B, b: 0n } => 2n | { a: #B, b: k } => 3n | w => 4n end",
        "let f = fn e => match e with \
         | { a: #A, b: 0n } => 1n | { a: #B, b: x } => 2n | { a: #A, b: k } => 3n | w => 4n end",
        "let f = fn e => match e with \
         | #T { a: #A, b: x } => 1n | #T { a: #B, b: 0n } => 2n | #T { a: #B, b: k } => 3n | w => 4n end",
    ] {
        let (_, out, output) = infer_src(src);
        assert!(out.errors.is_empty(), "{src}: {:#?}", out.errors);
        assert!(output.errors().is_empty(), "{src}: {:#?}", output.errors());
    ...
    let (_, _, output) = infer_src(
        "let wants_nat : Nat -> Nat = fn n => n\n\
         let f = fn e => match e with \
         | { a: #A, b: #X #Y } => 1n | { a: #A, b: k } => wants_nat {} end",
    );
    assert_eq!(output.errors().len(), 1, "{:#?}", output.errors());
    assert!(matches!(
        output.errors()[0].kind,
...
fn a_sibling_tags_positions_stay_open_under_the_catch_all() {
    let (mint, _, output) = inferred(
        "let f = fn e => match e with \
         | #A #B #C 0n => 1n | #A #B r => 2n | #D #E 0n => 4n | q => 5n end\n\
         let user = f (#D #F)\n\
         let deep = f (#D (#E 7n))",
    );
    assert_eq!(
...
fn a_wildcard_fn_argument_still_has_a_domain() {
    let (mint, _, output) = inferred("let f = fn _ => 1n");
    ...
    let (mint, _, output) = inferred("let f = fn _ _ => 1n  let g = fn _ x _ => x");
    ...
    let (mint, _, output) = inferred("let f = fn _ _ => 1n  let n = f 1n {}");
    assert_eq!(scheme(&mint, &output, "n"), "Nat");
}
...
fn a_discarded_value_is_still_typechecked() {
    ...
    let (_, _, output) = infer_src("let _ = 1n 2n");
    assert_eq!(output.errors().len(), 1, "{:#?}", output.errors());
    assert!(matches!(
        output.errors()[0].kind,
        ErrorKind::Mismatch { .. }
...
fn a_wildcard_arm_types_as_a_named_catch_all() {
    let with_wildcard = "type Option 'T = #Some 'T | #None\n\
                         let get = fn opt => match opt with | #Some x => x | _ => 0n end";
    let with_name = "type Option 'T = #Some 'T | #None\n\
                     let get = fn opt => match opt with | #Some x => x | z => 0n end";
    ...
    let (mint, _, output) = inferred(
        "type Option 'T = #Some 'T | #None\n\
         let get : Option Nat -> Nat = fn opt => match opt with | #Some x => x | _ => 0n end",
    );
...
fn an_exact_let_pattern_is_exact() {
    let (_, _, output) = infer_src("let q = {x: 1n, y: 2n}  let {x} = q");
    assert_eq!(output.errors().len(), 1, "{:#?}", output.errors());
    match &output.errors()[0].kind {
    ...
    let (mint, _, output) = inferred("let {x, ..} = {x: 1n, y: 2n}");
...
fn a_mixed_match_is_one_ordinary_mismatch() {
    let (_, out, output) = infer_src("let f = fn e => match e with | 0n => 1n | #A x => 2n end");
    assert!(out.errors.is_empty(), "{:#?}", out.errors);
    assert_eq!(output.errors().len(), 1, "{:#?}", output.errors());
    assert!(matches!(
        output.errors()[0].kind,
...
fn a_case_tested_by_numbers_stays_present_for_the_catch_all() {
    let (mint, _, output) = inferred(
        "let keep = fn r => 0n\n\
         let f = fn e => match e with | #A 1n => 1n | #A 2n => 2n | r => keep r end",
    );
    assert_eq!(scheme(&mint, &output, "f"), "#A Nat | ..'a -> Nat");
}
...
fn nested_presence_guards_conjoin_and_name_nested_paths() {
    ...
    assert_eq!(
        output.diagnostics().refinements().len(),
        4,
        "{:#?}",
    ...
    assert!(
        output.diagnostics().refinements()[0]
            .fields
            .iter()
...
fn overlap_uses_exclusion_and_arm_assumptions_do_not_leak() {
    ...
    assert_eq!(output.diagnostics().refinements().len(), 2);
    let second = &output.diagnostics().refinements()[1];
    assert!(
        second
            .facts
            .iter()
...
fn refinement_stays_presence_only_and_mixed_matches_stay_flat() {
    ...
    assert!(output.diagnostics().refinements().is_empty());
    assert!(
        output
            .diagnostics()
            .constraints()
...
fn an_unreachable_arm_is_checked_without_its_contradictory_assumption() {
    ...
    assert_eq!(output.diagnostics().refinements().len(), 2);
    assert!(!output.diagnostics().refinements()[1].reachable);
}
...
fn a_guarded_presence_clash_becomes_a_false_obligation() {
    ...
    assert!(
        output.diagnostics().steps().iter().any(|step| {
            matches!(
                &step.effect,
...
fn a_clause_can_fold_a_presence_present() {
    let (mint, _, output) = inferred("let f : { x when 'a: Nat, .. } -> Nat where 'a = fn v => 1n");
    assert_eq!(scheme(&mint, &output, "f"), "{ x: Nat, ..'a } -> Nat");
    assert!(
        output
            .semantics()
...
fn an_annotation_its_definition_rules_out_is_refused() {
    let src = "let f : { x when 'a: Nat } -> Nat where 'a = fn v => match v with | {} => 1n end";
    let (mint, _, output) = infer_src(src);
    let [error] = output.errors() else {
        panic!("expected one error: {:#?}", output.errors());
    };
...
fn an_arms_body_is_the_operations_result() {
    let codes = infer_codes(&format!(
        "{EFFECTS}let p : () -> Nat + !Log = fn _ => 0n\n\
         let h = fn _ => handle p () with | !Log.write s => 1n end"
    ));
    assert_eq!(codes, ["type-mismatch"]);
}

/// A row of effects handed to a declaration as an argument is the row the
/// declaration's `..` stands for: it is spliced in where the parameter sat, and
/// what comes out is the arrow the reader would have written by hand.
///
/// The one place a row reaches inference without an arrow around it, so it is
/// the one place a set of labels arrives as an argument and has to be read as
/// the row it is.
#[test]
fn a_row_of_effects_may_be_handed_to_a_declaration() {
    let (mint, _, output) = inferred(&format!(
        "{EFFECTS}type Runner 'e = (Nat -> Nat + ..'e) -> Nat\n\
         let run : Runner (!Log) -> Nat = fn r => 1n\n\
         let both : Runner (!Log + !IO) -> Nat = fn r => 1n"
    ));
    // The scheme keeps the declaration nominal, and a nominal argument prints
    // as the bare set of labels it is: which of the three rows it will be
    // spliced into is the parameter's to say, and a printed type carries no
    // parameter along with it. Both spellings name the same set and both
    // re-parse, which is what keeps a printed scheme source.
    assert_eq!(scheme(&mint, &output, "run"), "Runner (#Log) -> Nat");
    assert_eq!(scheme(&mint, &output, "both"), "Runner (#Log | #IO) -> Nat");

    // Unfolded — which is what a complaint about the type shows — the row is
    // the arrow's own effects, and reads as the effects it is.
    let (_, _, output) = infer_src(&format!(
        "{EFFECTS}type Runner 'e = (Nat -> Nat + ..'e) -> Nat\n\
         let bad : Runner (!Log) = 1n"
    ));
    let [error] = output.errors() else {
        panic!("{:#?}", output.errors());
    };
    assert_eq!(
        error.kind.to_string(),
        "type mismatch: expected `(Nat -> Nat + !Log) -> Nat`, found `Nat`"
    );
}
...
fn presence_written_on_an_effect_survives_into_the_scheme() {
    let (mint, _, output) = inferred(&format!(
        "{EFFECTS}let f : Nat -> Nat + !Log (when 'a) + !IO (when 'b) where 'a != 'b\n\
         \x20 = fn x => x"
    ));
    assert_eq!(
        scheme(&mint, &output, "f"),
...
/// An annotation is the contract, and a declared `..'e` is a rigid: it is
/// quantified however few times it occurs — R23's closing rule counts solver
/// variables and a rigid is none — and a body that performs into it is refused
/// at the label it demanded, since the caller is who decides what `e` allows.
#[test]
...
fn an_effect_written_absent_lowers_to_an_absent_presence() {
    ...
    let symbol = symbol_named(&mint, output.semantics().schemes().keys().copied(), "f");
    let body = output.semantics().schemes()[&symbol].body().clone();
    let Ty::Arrow(_, _, effects) = &*body else {
        panic!("expected an arrow, got {body:?}");
    };
    let label = effects
        .labels
...
fn two_effect_rows_with_extras_both_ways_share_a_fresh_tail() {
    let (mint, _, output) = inferred(&format!(
        "{EFFECTS}let p : (Nat -> Nat + !Log + ..'r) -> Nat = fn f => 1n\n\
         let q : (Nat -> Nat + !IO + ..'r) -> Nat = fn f => 1n\n\
...
fn an_abandoned_effect_presence_prints_as_a_question() {
    let (_, _, output) = infer_src(&format!(
        "{EFFECTS}let f : Nat -> Nat + !Log (when 'a) = 1n"
    ));
    let quoted: Vec<String> = output
        .errors()
        .iter()
...
fn an_assumption_compares_the_arguments_it_was_pushed_at() {
    let (mint, _, output) = inferred(
        "type A 'a = { v: 'a, next: A (Nat -> {}) }\n\
         ...
         let f : A (Nat -> Nat) -> Nat = fn x => 1n\n\
         let g : B (Nat -> Nat) -> Nat = fn x => 1n\n\
         let h = fn v => { one: f v, two: g v }",
    );
    assert_eq!(
        scheme(&mint, &output, "h"),
...
fn a_refused_raise_still_walks_its_value() {
    let (mint, out, output) = infer_src("let a = raise 1n");
    assert_eq!(
...
fn a_raise_under_a_function_answers_no_arm() {
    let (mint, out, output) = infer_src(&format!(
        "{EFFECTS}let p : () -> Nat + !Log = fn _ => 0n\n\
         let h = fn _ => handle p () with | !Log.write s => {{ g: fn _ => raise 0n }} end"
    ));
    assert_eq!(
        out.errors.iter().map(|e| e.kind.code()).collect::<Vec<_>>(),
        ["raise-in-function"]
...
/// Every example the specification writes out, pinned by the scheme it
/// publishes. The whole point of the feature is in this list: an annotation
/// that can name its own variables, a body held to what it named, and holes
/// that still mean "infer this".
#[test]
fn the_written_examples_publish_what_they_say() {
    for (src, name, published) in [
        // Annotating the identity function, which was unwritable before.
        ("let id : 'a -> 'a = fn x => x", "id", "'a -> 'a"),
        // Two holes, inferred equal — the footgun the feature removes.
        (
            "let id : {..} -> {..} = fn a => a",
            "id",
            "{ ..'a } -> { ..'a }",
        ),
        // The same promise made rigidly, and kept.
        (
            "let id : { ..'r } -> { ..'r } = fn a => a",
            "id",
            "{ ..'a } -> { ..'a }",
        ),
        // A hole where a promise would be wrong: the bare `..` is the
        // definition's to decide, so reading a field through it is no
        // complaint.
        (
            "let fst : { x: 'a, .. } -> 'a = fn p => p.x",
            "fst",
            "{ x: 'a, ..'b } -> 'a",
        ),
        // Subsumption: a body more general than the annotation checks fine,
        // and the annotation is what is published.
        ("let f : Nat -> Nat = fn x => x", "f", "Nat -> Nat"),
        // A hole in a type position.
        ("let k : _ -> Nat = fn x => 0n", "k", "'a -> Nat"),
        // A statement list, mixed sorts, any order.
        (
            "let both : { x when 'a: 'c, y when 'b: 'd } -> {} where 'a = 'b\n\
             \x20 = fn v => match v with\n\
             \x20   | {x, y} => {}\n\
             \x20   | {} => {}\n\
             \x20 end",
            "both",
        ...
        let (mint, _, output) = inferred(src);
        assert_eq!(scheme(&mint, &output, name), published, "{src}");
    }
}

/// The annotation is the contract, so a use of the name sees it and not
/// whatever the body could have been inferred to be.
#[test]
fn a_specializing_annotation_is_what_a_use_sees() {
    let (mint, _, output) = inferred("let f : Nat -> Nat = fn x => x\nlet n = f 1n");
    assert_eq!(scheme(&mint, &output, "n"), "Nat");
...
/// Every example the specification writes out that is refused, pinned by its
/// code, its wording and the span it is reported at.
#[test]
fn the_written_examples_are_refused_where_they_go_wrong() {
    ...
    let read = output
        .errors()
        .iter()
        .find(|error| error.kind.code() == "rigid-field")
    ...
    assert_eq!(
        error.kind.to_string(),
        "this is `'a`, but `'b` stands for whatever type the caller picks"
    );
...
/// other core it can meet is the promise broken, and the complaint names what
/// the expression turned out to be.
#[test]
fn a_rigid_meets_only_itself() {
    // The same variable twice is already the same thing.
    let (mint, _, output) = inferred("let f : 'a -> 'a = fn x => x");
    assert_eq!(scheme(&mint, &output, "f"), "'a -> 'a");

    // Every other core, one at a time. `found` is what the body turned out to
    ...
    for (body, found) in [
        ...
        let src = format!("let f : 'a -> 'a = fn x => {body}");
        let (_, _, output) = infer_src(&src);
        let [error] = output.errors() else {
        ...
        assert!(
            error
                .kind
                .to_string()
                .starts_with(&format!("this is `{found}`")),
            "{src}: {}",
            error.kind
        );
    }

    // A declared name is not looked through first: what the reader wrote is
    // the name, and unfolding it would quote them a shape they never wrote.
    let (_, _, output) = infer_src(
        "type Endo = Nat -> Nat\n\
         let e : Endo = fn n => n\n\
         let f : 'a -> 'a = fn x => e",
    ...
    assert_eq!(
        error.kind.to_string(),
        "this is `Endo`, but `'a` stands for whatever type the caller picks"
    );

    // And an unbound variable takes the rigid, as it would take any type: that
    // is what makes a lambda argument checked against one work at all.
    let (mint, _, output) = inferred("let f : 'a -> 'a = fn x => x");
    assert_eq!(scheme(&mint, &output, "f"), "'a -> 'a");
}

/// A label demanded of a declared variable is refused outright, whichever of
/// the two sorts a rest can be: nothing can ever be bound to a rigid, so there
/// is no lacks condition to record and nothing to record it on.
#[test]
fn a_label_demanded_of_a_rigid_is_refused() {
    // A whole type, read for a field.
    let (_, _, output) = infer_src("let f : 'a -> Nat = fn x => x.y");
    let [error] = output.errors() else {
        panic!("expected one error: {:#?}", output.errors());
    };
    assert_eq!(error.kind.code(), "rigid-field");
    ...
    // A struct's rest, read for a field its row does not name.
    let (_, _, output) = infer_src("let f : { x: Nat, ..'r } -> Nat = fn p => p.y");
    let [error] = output.errors() else {
        panic!("expected one error: {:#?}", output.errors());
    };
    assert_eq!(error.kind.code(), "rigid-field");
...
/// Inside its own body an annotated definition's name is a scheme quantifying
/// what the annotation declared and nothing else. So a recursive use is
/// polymorphic in the declared variables and monomorphic in the holes, which is
/// the whole of R29 in two programs.
#[test]
fn a_recursive_use_instantiates_what_was_declared() {
    // Polymorphic in what was declared: the second mention of `r` is a fresh
    // copy, so handing it a closed record is no conflict.
    let (mint, _, output) = inferred(
        "type Nest = { kids: Nest }\n\
         let depth : { kids: Nest, ..'r } -> Nat = fn n => depth n.kids",
    );
    ...
    assert_eq!(scheme(&mint, &output, "f"), "Nat -> Nat");
    let (_, _, output) = infer_src(
        "let keep = fn a => fn b => a\n\
         let f : _ -> Nat = fn x => keep (f 1n) (f {})",
    );
    assert_eq!(output.errors().len(), 1, "{:#?}", output.errors());
    assert_eq!(output.errors()[0].kind.code(), "type-mismatch");
}

/// A nested `let` is the same feature about a smaller scope: its annotation
/// declares its own variables, its body is held to them, and its holes are
/// still the value's to decide.
#[test]
fn a_nested_annotation_declares_its_own_variables() {
    ...
    let local = output
        ...
        .values()
        .next()
        .expect("the binding published a scheme");
...
/// A sum's rest goes by the core rule's arms, said about cases: a declared rest
/// is equal to itself, takes an unbound tail as any row would, and breaks its
/// promise against anything that allows nothing more.
#[test]
fn a_declared_sum_rest_is_equal_to_itself_and_to_nothing_else() {
    // The same rest on both sides of an arrow is one promise kept.
    let (mint, _, output) = inferred("let f : (#A Nat | ..'r) -> (#A Nat | ..'r) = fn p => p");
    assert_eq!(
        scheme(&mint, &output, "f"),
        "#A Nat | ..'a -> #A Nat | ..'a"
    );

    // And it is refused against a sum that allows nothing more, whichever side
    // of the goal the promise sits on. Here it is the expected one: the
    // argument written at `k` is what the annotation says `k` takes.
    let (_, _, output) = infer_src(
        "let closed : #A Nat = #A 1n\n\
         let f : ((#A Nat | ..'r) -> Nat) -> Nat = fn k => k closed",
    );
    let [error] = output.errors() else {
        panic!("expected one error: {:#?}", output.errors());
    };
...
/// A rigid takes nothing and is taken by nothing but an unbound variable, and
/// the tail rules say so in both sorts. What is left is the pairing that binds:
/// a declared rest meeting a tail the solve still owns, which takes the rigid
/// as it would take any other rest.
#[test]
fn a_variable_tail_takes_a_declared_rest() {
    // A struct's rest, where the two rows name the same fields and the tails
    // are what is left to agree.
    let (mint, _, output) = inferred(
        "let f : ({ x: Nat, ..'r } -> Nat) -> { x: Nat, .. } -> Nat = \
         fn k => fn v => k v",
    );
    assert_eq!(
        scheme(&mint, &output, "f"),
        "({ x: Nat, ..'a } -> Nat) -> { x: Nat, ..'a } -> Nat"
    );

    // And a sum's, which is the same rule about cases.
    let (mint, _, output) = inferred(
        "let g : ((#A Nat | ..'r) -> Nat) -> (#A Nat | ..) -> Nat = \
         fn k => fn v => k v",
    );
    assert_eq!(
        scheme(&mint, &output, "g"),
        "(#A Nat | ..'a -> Nat) -> #A Nat | ..'a -> Nat"
    );

    // Two declared rests are two promises, and one is no more the other than
    // a closed row is.
    let (_, _, output) = infer_src("let h : (#A Nat | ..'r) -> (#A Nat | ..'s) = fn p => p");
    let [error] = output.errors() else {
        panic!("expected one error: {:#?}", output.errors());
    };
    assert_eq!(error.kind.code(), "rigid-broken");
...
fn extern_boundary_errors_publish_exact_source_payloads_and_paths() {
    ...
    let [error] = output.errors() else {
        panic!(
            "expected one callback boundary error: {:#?}",
            output.errors()
        );
    };
...
#[test]
fn explicit_variant_edges_are_solved_in_both_directions() {
    let (_, _, output) =
        infer_src("let use : Nat -> Nat = fn n => n\nlet bad : 'a -> Nat = fn x => use x");
    assert!(
        output
            .errors()
            .iter()
            .any(|error| error.kind.code() == "rigid-broken")
    );

    for src in [
        "let mixed = match true with | true => {} | false => 1n end",
        "let mixed = match true with | false => 1n | true => {} end",
        "let mixed = match true with | true => {} | false => #A end",
    ] {
        let (_, _, output) = infer_src(src);
        assert!(
            output
...
fn inference_records_have_unique_direct_identities_across_nested_constraints() {
    ...
    let step_ids: HashSet<_> = output
        .diagnostics()
        .steps()
        .iter()
        .map(|step| step.id)
        .collect();
    assert_eq!(step_ids.len(), output.diagnostics().steps().len());
    assert!(output.diagnostics().steps().iter().all(|step| {
        step.constraint
            .is_none_or(|constraint| constraint_ids.contains(&constraint))
    }));

    let batch_ids: HashSet<_> = output
        .semantics()
        .store()
        .batches
        .iter()
        .map(|batch| batch.id)
        .collect();
    assert_eq!(batch_ids.len(), output.semantics().store().batches.len());
...
fn callback_steps_resolve_to_published_callback_constraints() {
    ...
    let callback_ids: HashSet<_> = callbacks
        ...
        .collect();
    assert!(
        !callback_ids.is_empty(),
        "{:#?}",
        output.diagnostics().constraints()
    );
    assert!(
        output
            .diagnostics()
...
fn direct_boundary_errors_have_stable_ids_without_solve_steps() {
    ...
    assert!(
        output
            ...
            .steps()
            .iter()
            .all(|step| step.error != Some(error.id))
    );
}
...
fn variables_and_solver_changes_link_into_the_immutable_reason_arena() {
    ...
    assert!(!output.diagnostics().variables().is_empty());
    let ids: HashSet<_> = output
        ...
        .collect();
    assert_eq!(ids.len(), output.diagnostics().reasons().len());
    assert!(
        output
            .diagnostics()
            .reasons()
...
fn sat_and_effect_defaults_are_first_class_binding_reasons() {
    ...
    let arena: std::collections::HashMap<_, _> = output
        ...
        .collect();
    for reason in output.diagnostics().reasons().iter().filter(|reason| {
        matches!(
            reason.origin,
            inference::ReasonOrigin::DefaultBinding { .. }
        )
...
fn replaced_and_guarded_batches_keep_referentially_integral_reason_roots() {
    ...
    let reasons: std::collections::HashMap<_, _> = local
        ...
        .collect();
    let mut constraints = Vec::new();
    for generated in local.diagnostics().constraints().values() {
        all_constraints(generated, &mut constraints);
    }
    ...
    assert!(reason.parents.iter().any(|parent| matches!(
        ...
        inference::ReasonOrigin::Batch(id)
            if guarded.semantics().store().batches.iter().any(|batch| {
...
fn a_failure_slice_crosses_the_constraints_whose_bindings_it_resolved() {
    ...
    let slice = output.diagnostics().reason_ancestors(failure.reason);
...
fn published_reason_links_never_reach_rolled_back_origins() {
    ...
    for reason in output
        ...
        .reasons()
        .iter()
        .filter(|reason| reason.reachable)
    {
        assert!(
            reason
                .parents
        ...
        if let inference::ReasonOrigin::Step(step) = reason.origin {
            assert!(
                output
                    ...
                    .steps()
                    .iter()
                    .any(|candidate| candidate.id == step)
            );
        }
    }
}
