...
impl fmt::Display for Shown<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "@{}", self.key)?;
        match unit_data(&self.attribute.value.anchored) {
            true => Ok(()),
            false => write!(f, " {}", data(&self.attribute.value.anchored, self.mint)),
        }
    }
}
...
impl Grouped for Show<'_, DataKind> {
    fn prec(&self) -> Prec {
        match self.node {
            DataKind::Tag { payload, .. } if !unit_data(&payload.anchored) => Prec::Apply,
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self.node {
            ...
            DataKind::Tag { name, payload } => write_tag(
                ...
                name,
                None,
                (!unit_data(&payload.anchored)).then(|| self.show(payload.as_ref())),
            ),
        }
    }
}
...
impl Grouped for Show<'_, TypeKind> {
    fn prec(&self) -> Prec {
        match self.node {
            ...
            | TypeKind::Mut(..)
            | TypeKind::Mirror(_)
            | TypeKind::TypeInfo(_) => Prec::Apply,
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self.node {
            TypeKind::Structural {
                ...
                f.write_str(&ruddy::ui::structural_type_text_with_arguments(
                    *parameters,
                    body,
                    &captures
                        .iter()
                        .map(|ty| self.show(ty).to_string())
                        .collect::<Vec<_>>(),
                    &arguments
                        .iter()
                        .map(|ty| self.show(ty).to_string())
                ...
                if let Some(sink) = effect_sink
                    && let TypeKind::Arrow { effects, .. } = &sink.anchored
                    && let Some(effects) = self.effect_row(effects)
                {
                    write!(f, " + {effects}")?;
                }
                Ok(())
            ...
            TypeKind::Match(arms) => {
                f.write_str("match")?;
                for (from, to) in arms {
                    let grouped = matches!(&from.anchored, TypeKind::Sum { cases, spreads, tail }
                        if cases.len() != 1 || !spreads.is_empty() || tail.is_some());
                    f.write_str(" | ")?;
                    if grouped {
                        f.write_str("(")?;
                    }
                    write!(f, "{}", self.show(from))?;
                    if grouped {
                        f.write_str(")")?;
                    }
                    // A result can end in an unclosed `fn | ...`, including
                    // through an arrow or `hide`. Keep the next arm outside it.
                    let shown = self.show(to);
                    let grouped = shown.prec() <= Prec::Arrow;
                    f.write_str(" => ")?;
                    if grouped {
                        f.write_str("(")?;
                    }
            ...
            TypeKind::TypeInfo(element) => {
                write_applied(f, "TypeInfo", std::iter::once(self.show(&**element)))
            }
