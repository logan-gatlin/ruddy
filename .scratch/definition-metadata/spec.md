...
- Attributes with no definition after them are reported at the span from the first attribute to the last, as metadata with nothing to describe. Recovery continues at the token that stopped the statement so that token's own complaint, if it has one, is still made.
...
- The debugger's surface printer prints each attribute on its own line before the definition it belongs to, key first, then the value when it is not unit, using the existing string, field-label, and tag-label writers. The normalized printer prints a declaration's metadata the same way. A unit value prints as the bare attribute.
...
- Rejected compilation tests cover a repeated key, a duplicate field inside a metadata struct, a non-literal value at the top of a value and inside a struct field, an attribute at end of file, an attribute before `end`, an attribute on a block-level `let`, an attribute in expression position, and `@` alone.
