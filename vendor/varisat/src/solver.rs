...
impl<'a> Solver<'a> {
    ...
    /// Check cancellation between search steps. Unwinding the callback abandons
    /// this solver; the caller must discard it, rather than resume partial work.
    pub fn solve_with(&mut self, mut checkpoint: impl FnMut()) -> Result<bool, SolverError> {
        ...
        loop {
            checkpoint();
            if !schedule_step(ctx.borrow()) { break; }
        }
